...
import { Product, Category, CURRENCIES, MeasurementType, WeightUnit, WEIGHT_UNITS, Currency } from '@/types';
import { formatMeasurement, getMeasurementUnit, isLowStock, isOutOfStock, getStockStatus, getInputStep } from '@/utils/measurement';
...
export default function Warehouse() {
  const { state, addProduct, updateProduct, deleteProduct, addCategory, updateCategory, deleteCategory } = useApp();
  ...
  const handleCategorySubmit = (e: React.FormEvent) => {
    ...
    const categoryData = {
      ...
      subcategories: categoryFormData.subcategories.map((sub, index) => ({
        id: `${Date.now()}-${index}`,
        name: sub.name,
  ...
  const addSubcategory = () => {
    setCategoryFormData({
      ...categoryFormData,
      subcategories: [...categoryFormData.subcategories, { name: '', description: '' }],
    });
  };
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">المستودع</h1>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setFormData({ ...formData, category: value === "placeholder" ? "" : value, subcategory: '' })}
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="none">لا توجد فئة فرعية</SelectItem>
                      {categories
              ...
              <div className="space-y-4 p-4 bg-gray-50 rounded-lg border-2 border-gray-200">
                <div className="space-y-2">
                  <Label className="text-base font-semibold">نوع القياس *</Label>
                  <div className="flex gap-4">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        onChange={(e) => setFormData({ ...formData, measurementType: 'quantity' })}
                        className="w-4 h-4 text-blue-600"
                      />
                      <span className="text-sm font-medium">بالقطع (عدد)</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="radio"
                        ...
                        onChange={(e) => setFormData({ ...formData, measurementType: 'weight' })}
                        className="w-4 h-4 text-blue-600"
                      />
                ...
                {formData.measurementType === 'quantity' ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="quantity">الكمية المتوفرة (قطعة) *</Label>
                      <Input
                        id="quantity"
                        type="number"
                        required
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setFormData({ ...formData, weightUnit: value as WeightUnit })}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label htmlFor="weight">الوزن المتوفر ({WEIGHT_UNITS[formData.weightUnit].symbol}) *</Label>
                        <Input
                          id="weight"
                          type="number"
                          required
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="minWeight">الحد الأدنى للوزن ({WEIGHT_UNITS[formData.weightUnit].symbol}) *</Label>
                        <Input
                          id="minWeight"
                          type="number"
                          required
      ...
      <Card>
                    ...
                    <TableRow key={product.id}>
                      <TableCell className="text-right">
                        <div>
                          ...
                          {product.barcode && (
                            <p className="text-xs text-muted-foreground">الباركود: {product.barcode}</p>
                          )}
                        </div>
                      </TableCell>
                      <TableCell className="text-right">
                      ...
                      <TableCell className="text-right">
                        <Badge variant="outline" className={product.measurementType === 'quantity' ? "bg-blue-50 text-blue-700 border-blue-200" : "bg-purple-50 text-purple-700 border-purple-200"}>
                          {product.measurementType === 'quantity' ? 'قطع' : `وزن (${getMeasurementUnit(product)})`}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-right">{formatCurrency(product.wholesalePrice, product.currency)}</TableCell>
                      <TableCell className="text-right">{formatCurrency(product.salePrice, product.currency)}</TableCell>
                      <TableCell className="text-right">
                        <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                          {CURRENCIES[product.currency].symbol} {CURRENCIES[product.currency].name}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-right">
                        {product.discount > 0 && (
        ...
        <TabsContent value="categories" className="space-y-6">
                ...
                <form onSubmit={handleCategorySubmit} className="space-y-4">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <Label className="text-base font-medium">الفئات الفرعية</Label>
                        <Button
                          type="button"
                          variant="outline"
                          size="sm"
                      ...
                      {categoryFormData.subcategories.map((sub, index) => (
                        <div key={index} className="bg-gray-50 p-4 rounded-lg border">
                          <div className="flex items-center justify-between mb-3">
                            <span className="font-medium text-gray-700">فئة فرعية {index + 1}</span>
                            <Button
                              type="button"
                              variant="ghost"
                              size="sm"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories.map((category) => (
              <Card key={category.id} className="hover:shadow-lg transition-all duration-200 border-0 bg-gradient-to-br from-white to-gray-50/50">
                      ...
                      {category.subcategories.length > 0 ? (
                        <div className="space-y-2">
                          {category.subcategories.map((sub) => (
                            <div key={sub.id} className="bg-blue-50 border border-blue-100 p-2 rounded-lg">
                              <p className="text-sm font-medium text-blue-800">{sub.name}</p>
                              {sub.description && (
                                <p className="text-xs text-blue-600">{sub.description}</p>
                              )}
                            </div>
                          ))}
                        </div>
                      ) : (
                        <p className="text-sm text-gray-500">لا توجد فئات فرعية</p>
                      )}
                    </div>
          ...
          {categories.length === 0 && (
            <Card>
              <CardContent className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">لا توجد فئات بعد</h3>
                <p className="text-gray-600 mb-4">ابدأ بإضافة فئة جديدة لتنظيم منتجاتك</p>
