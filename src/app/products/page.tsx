...
export default function ProductsAndSales() {
  ...
  const { addItem: addToCart, items: cartItems, openCart, isOpen: isCartOpen } = useCart();
  ...
  const handleAddToCart = (product: Product) => {
    try {
      ...
      const defaultWeight = product.measurementType === 'weight' ? 0.001 : undefined; // Minimal placeholder for weight products
      ...
      if (product.measurementType === 'weight') {
        openCart();
        if (existingItem) {
          toast.success(`تم زيادة ${product.name} - افتح السلة لتعديل الوزن`);
        } else {
          toast.success(`تمت إضافة ${product.name} - افتح السلة لإدخال الوزن`);
        }
  ...
  const handleBarcodeScanned = (barcode: string) => {
    ...
    if (product) {
      ...
      if (isOutOfStock(product)) {
        ...
        // Add product directly to cart instead of opening sale modal
  ...
  // Auto-focus barcode input on mount and after modal/cart closes
  useEffect(() => {
    if (!isSaleModalOpen && !isDetailsModalOpen && !isReturnModalOpen && !isCartOpen) {
  ...
  }, [isSaleModalOpen, isDetailsModalOpen, isReturnModalOpen, isCartOpen]);
                ...
                {selectedProduct.measurementType === "quantity" ? (
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      value={saleFormData.quantity === 0 ? "" : saleFormData.quantity}
                      onChange={(e) =>
                        setSaleFormData({
                          ...saleFormData,
